...
本 Python 项目使用标准库 `cProfile` 和 `pstats` 进行可复现的性能分析；它们不需要网络或第三方包。基准输入为重复的中文测试文本，原文和抄袭版各约 190,000 个字符。
...
在本机对 `profile_benchmark.py` 运行一次的结果为 0.107 秒、408,841 次函数调用。累计耗时最高的自有函数是 `build_ngram_frequency`（两次调用累计约 0.092 秒），其次是 `calculate_similarity`（约 0.097 秒）。这说明主要成本来自构造 N-gram 词频，而不是余弦点积；当前实现已经避免了额外构造两个向量键的并集。
